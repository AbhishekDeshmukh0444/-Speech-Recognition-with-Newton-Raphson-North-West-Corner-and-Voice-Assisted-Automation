...
def method():
    ...
    def answer(l):
        ...
        for entries in supply:
            ...
            # my_label.config(text=entry_list2)

        for entries in cost:
            entry_list1 = entry_list1 + ',' + entries.get()
            # my_label.config(text=entry_list1)

        for entries in demand:
            entry_list3 = entry_list3 + ',' + entries.get()
            # my_label.config(text=entry_list3)

        # For supply list

        # print(entry_list2)
        # print(type(entry_list2))
        x2 = entry_list2.split(",")
        # print(x2)
        x2.pop(0)
        # print(x2)
        res2 = [eval(i) for i in x2]
        # print("Modified list of supply is:", res2)

        # For demand list

        # print(entry_list3)
        # print(type(entry_list3))
        x3 = entry_list3.split(",")
        # print(x2)
        x3.pop(0)
        # print(x2)
        res3 = [eval(i) for i in x3]
        # print("Modified list of demand is:", res3)

        # For cost list

        # print(entry_list1)
        # print(type(entry_list1))
        x1 = entry_list1.split(",")
        # print(x1)
        x1.pop(0)
        # print(x1)
        res1 = [eval(i) for i in x1]
        # print("Modified list of cost is:", res1)
    ...
    def matrix(r, c):
        ...
        for i in range(r1 + 2, r1 + 3):
            b3 = Button(f2, text="Answer", command=lambda: answer(label2), font=("Cascadia Code SemiBold", 10),
    ...
    def home():
        ...
        Label(f1, text="North West Corner Method", bg="light blue", font=("Bahnschrift SemiBold", 18)).grid(row=1,
                                                                                                            column=1,
                                                                                                            padx=10)
        Label(f1, text="  Number of rows:  ", bg="light blue", font=("Segoe UI Variable Display Semib", 13)).grid(row=2,
                                                                                                                  column=0,
                                                                                                                  padx=20,
                                                                                                                  pady=20)
        Label(f1, text="Number of column:", bg="light blue", font=("Segoe UI Variable Display Semib", 13)).grid(row=3,
                                                                                                                column=0)
        ...
        # Button
        Button(f1, text="Generate", font=("Cascadia Code SemiBold", 10), activebackground="white",
               activeforeground="red", command=lambda: matrix(rows.get(), columns.get())).grid(row=6, column=1, pady=20)
...
def newton():
    ...
    eqn = Label(root, text="Enter equation with variable x : ", font="Georgia 15",
                bg="#EBDEF0")  # use * for multiplication and ** for power
    x_0 = Label(root, text="Enter the initial guess (real no.): ", font="Georgia 15",
                bg="#EBDEF0")  # it can be any number according to the graph of eqn, mostly it is one
    eqn.place(x=80, y=60)
    ...
    def calculator():
        ...
        try:
            ...
            for i in range(10):
                ...
                x1 = round(x1, 3)
                err = (exp / der).subs(x, x0)
            Label(root, text=f'Root for x is {round(x1, 3)}', font="arial 30 bold", bg="#EBDEF0").place(x=220,
                                                                                                        y=250)  # output on window
        except:
